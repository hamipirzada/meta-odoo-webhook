...
# Root route for webhook verification
@app.route('/', methods=['GET'])
def verify_webhook_root():
    """Handle webhook GET challenge for verification at root"""
    mode = request.args.get('hub.mode')
    token = request.args.get('hub.verify_token')
    challenge = request.args.get('hub.challenge')

    print("🔍 Verification requested at root")
...
def verify_webhook():
    """Handle webhook GET challenge for verification"""
    mode = request.args.get('hub.mode')
    token = request.args.get('hub.verify_token')
    challenge = request.args.get('hub.challenge')
...
def receive_webhook():
    ...
    try:
        ...
        # Signature verification (disabled for debugging - re-enable for production)
...
def test_endpoint():
    ...
    return jsonify({
        ...
        "webhook_url": request.host_url + "api/webhook"
    })
